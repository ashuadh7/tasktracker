...
1. **pull** — phone rows the desktop has never seen. This is the one thing that stays manual: Ashu fetches from Notion himself when he wants the desktop to ingest phone entries. A non-empty pull list is a prompt to ask him, never something to merge on its own — merging it first would make the next step's `create`/`delete` wrong anyway.
   ...
   **A pulled row is a bare capture, not a finished one — the phone stays quick and dirty on purpose, so the enrichment a live desktop session would normally do (this file's own tagging rules, a growth-ledger row, a progress-log update) hasn't happened yet.** Before merging, check each pulled row against what it would need if it had been logged live: a `targeted_work` row needs its `target` (see "Tagging targeted_work to a target" below — check `plan.csv` for that day/time same as always); an activity that sounds like an audiobook, a walk, a novel, or similar needs a `growth-log.csv` row; a mentioned percent needs a `progress-log.csv` row. Probe Ashu for whatever's missing as part of the ingestion conversation — the same one question, batched, no-interrogation discipline as any other logging pass — rather than merging the row bare and letting the gap sit there uncaught the way it would have to be reconstructed later.
...
`activity` was never enough to tell which planned item a `targeted_work` row was actually for — `P03`, `reading and writing`, `Throughline work` are labels, not a pointer at a specific row in `targets.csv`. That gap is what made the completion index's click-through (see the visualizer) unable to show more than a project-wide guess, and what made "when was this actually done, versus when I planned it" impossible to draw on the timeline at all. The `target` column fixes it: **every `targeted_work` row gets the exact `targets.csv` `target` string it was toward**, filled in the same pass as everything else in step 2 of the loop, not asked about separately.

- **Probe now, not later.** The whole reason this column exists is to stop reconstructing after the fact — a guess made weeks later from a vague `activity` label is exactly the failure mode the tag is meant to kill. If it's not obvious which target a block was for, ask in the moment, the same way an unclear duration or a residual gets asked about. A quick "NSERC or Throughline?" while he still remembers costs one message; resolving it later costs a research project.
- **Check `plan.csv` first when it's ambiguous.** Once a target's planned blocks are in `plan.csv` (see `planning-workflow.md`), a log row's own date and time are the first thing to cross-reference against them — "you logged 7-9am Throughline, and 7-9am was planned as *formative study analysis* — that one?" is a much better question than a blind guess, and often means there's nothing to ask at all because only one target was scheduled there.
- **Ask, don't guess, when it's still not obvious.** If exactly one active target for that project fits what he described (from the dump or from `plan.csv`), fill it in and say so rather than asking. If several could fit, ask which one — batched with the rest of that pass's questions, same as any other ambiguity.
...
- **Rule took effect 2026-08-22; rows before that predate the column and stay blank**, same as the midnight-crossing sleep rule predating Aug 15 — `check.py` only enforces the tag from the cutover on (see `README.md`'s `time-log.csv` columns). **One exception on the books**: Aug 15-21 was backfilled once, by hand, row by row with Ashu confirming each ambiguous one — not silently guessed — because the click-through and the planned-vs-actual timeline needed real data to prove out against. That backfill is a one-time event, not a standing practice; nothing before the cutover gets touched again, and nothing after it should ever need backfilling if probing-now is actually followed.
