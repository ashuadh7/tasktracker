...
# -- the target-completion ramp -------------------------------------------
# Position on this ramp is priority: which target a project's plan lists
# first, the one everything else in that section depends on going first.
# That is what makes it a genuine ordinal ramp rather than a decorative
# one -- swapping two targets' order changes what the colour says, the
# same test that keeps every fixed palette above it from drifting.
# Chosen 2026-08-22 over a blue -> orange alternative (kept as a backup,
# see issue history) via an interactive picker -- see parking-lot.md #13.
TARGET_HUE_RANGE = (8, 145)   # red -> green
TARGET_BASE_S, TARGET_BASE_L = 0.56, 0.46
TARGET_MUTED_SAT_MUL = 0.25   # 75% desaturated
TARGET_MUTED_LIGHT_MUL = 0.90  # 10% darker


def target_hue(position, count):
    """Position `position` of `count` (0-indexed) along the ramp. A single
    target lands at the red (most-foundational) end rather than the middle."""
    t = position / (count - 1) if count > 1 else 0.0
    h0, h1 = TARGET_HUE_RANGE
    return _hsl_to_hex(h0 + (h1 - h0) * t, TARGET_BASE_S, TARGET_BASE_L)


def target_muted(hex_color):
    """The 'still to do' half of a target's segment -- same hue, pulled down
    in saturation and lightness so 'done' reads as the brighter of the two
    without the pair losing their shared identity."""
    h, s, l = _hex_to_hsl(hex_color)
    return _hsl_to_hex(h, s * TARGET_MUTED_SAT_MUL,
                       max(0.10, l * TARGET_MUTED_LIGHT_MUL))
